...
+  public Pistol2(int capacity)
      Вызывает конструктор родительского класса super(0), устанавливая начальное количество патронов в 0 (пистолет разряжен);
      Если вместимость отрицательная — выбрасывается IllegalArgumentException;
      Сохраняет значение вместимости в поле capacity.

*Методы*:
+  public void shoot() - выполняет один выстрел.
      Если есть патроны (getAmmo() возвращает true) — выводит "Бах!";
      Если патронов нет — выводит "Клац!".
+  public void shoot(int times) - выполняет несколько выстрелов подряд.
      Если times отрицательное — выбрасывается IllegalArgumentException;
      В цикле вызывает метод shoot() указанное количество раз.
+  public int reload(int bulletsToLoad) - перезаряжает пистолет.
      Если количество патронов отрицательное — выбрасывается IllegalArgumentException;
      Определяет текущее количество патронов (ammo());
      Вычисляет свободное место в магазине;
      Если патроны помещаются полностью — загружает их и возвращает 0;
      Если не помещаются — заполняет магазин до максимума и возвращает количество лишних патронов.
+  public int unload() - разряжает пистолет.
      Сохраняет текущее количество патронов;
      Устанавливает количество патронов в 0 через load(0);
      Возвращает количество извлечённых патронов.
+  public boolean isLoaded() - проверяет, заряжен ли пистолет (есть ли хотя бы один патрон).
...
+  public Weapon(int ammo) - создаёт оружие с заданным количеством патронов.
      Если передано отрицательное значение — выбрасывается RuntimeException;
      Иначе устанавливает начальное количество патронов.
...
+  public boolean getAmmo() - пытается использовать один патрон:
      Если патронов нет (ammo == 0) — возвращает false;
      Если патроны есть — уменьшает их количество на 1 и возвращает true.
      Таким образом, метод одновременно проверяет наличие патрона и расходует его.
+  public int load(int ammo) - устанавливает новое количество патронов:
      Если передано отрицательное значение — выбрасывается RuntimeException;
      Сохраняет текущее количество патронов во временную переменную;
      Обновляет поле ammo;
      Возвращает предыдущее количество патронов.
...
+  public Shooter(String name) - создаёт стрелка с заданным именем.
      Если имя null или пустое — выбрасывается IllegalArgumentException;
      Иначе сохраняет имя в поле name.

*Методы*:
+  Геттеры и сеттеры для полей name и weapon.
+  public void shoot() - выполняет выстрел:
      Если оружие установлено — выводит имя стрелка и вызывает метод shoot() у оружия;
      Если оружие отсутствует — выводит сообщение "Имя: не могу участвовать в перестрелке".
+  Переопределенный toString() - формирует строковое представление.
...
+  public AutomaticGun2(int ammo, int rateOfFire) - создаёт автомат:
      Передаёт начальное количество патронов в конструктор родительского класса Weapon;
      Если скорострельность меньше или равна 0 — выбрасывается IllegalArgumentException;
      Иначе устанавливает значение rateOfFire.

*Методы*:
+  public void shoot() - выполняет автоматическую очередь выстрелов.
      Выполняет цикл rateOfFire раз;
      При каждом шаге вызывает getAmmo():
         + Если патрон есть — выводит "Бах!";
         + Если патронов нет — выводит "Клац!" и прекращает стрельбу (break).
+  public void shoot(int seconds) - стрельба в течение заданного количества секунд.
      Если seconds меньше или равно 0 — выбрасывается IllegalArgumentException;
      Общее количество попыток выстрела равно seconds * rateOfFire;
      Стрельба прекращается досрочно при отсутствии патронов.
...
+  @Override метод public boolean equals(Object obj) - переопределённый метод сравнения объектов:
      Если сравниваются ссылки на один и тот же объект — возвращает true;
      Если объект не является экземпляром Point — возвращает false;
      Иначе сравнивает координаты x и y с помощью Double.compare(), что корректно для сравнения чисел типа double.
