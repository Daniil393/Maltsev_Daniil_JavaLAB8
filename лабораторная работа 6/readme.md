...
В ходе выполнения данного задания был разработан класс InvokeProcessorTest.

### InvokeProcessorTest
Класс предназначен для тестирования корректности работы обработчика аннотации @Invoke — InvokeProcessor.

Описание:
   - Проверяет обнаружение методов, помеченных аннотацией @Invoke, с использованием Reflection API.
   - Проверяет корректность вызова найденных методов.
   - Проверяет изменение состояния объекта после вызова аннотированных методов.
   - Использует фреймворк JUnit 5.

*Поле testSubject:*

Описание:
   - Объект тестируемого класса TestSubject.
   - Используется как экземпляр, передаваемый в InvokeProcessor.

*Метод setUp():*

Описание:
   - Создаёт новый экземпляр TestSubject перед выполнением каждого теста.
   - Гарантирует независимость тестов и сброс состояния объекта с помощью аннотации @BeforeEach.

*Метод shouldFindMethodsAnnotatedWithInvoke():*

Аннотация: @Test
Описание:

Получает все методы класса через getDeclaredMethods().
   - Проверяет наличие аннотации @Invoke с помощью isAnnotationPresent().
   - Добавляет найденные методы в список.
   - Проверяет, что найдено ровно 2 метода с аннотацией.

Назначение:
   - Подтверждает корректную работу механизма обнаружения аннотированных методов.

*Метод shouldInvokeAnnotatedMethodsAndChangeState():*

Аннотация: @Test

Описание:

   - Проверяет, что до обработки все флаги вызова методов равны false.
   - Вызывает InvokeProcessor.process(testSubject).
   - Проверяет:
      + Методы с аннотацией @Invoke были вызваны.
      + Их флаги установлены в true.
      + Метод без аннотации не был вызван.

   - Используемые проверки:
      + assertDoesNotThrow() — проверка отсутствия исключений.
      + assertTrue() — проверка успешного вызова нужных методов.
      + assertFalse() — проверка, что неаннотированный метод не был вызван.

   - Назначение:
      + Подтверждает корректную работу механизма автоматического вызова методов через рефлексию.

*Вспомогательный класс TestSubject:*

Описание:
   - Используется исключительно для тестирования InvokeProcessor.
   - Содержит три метода:
      + methodOne() — помечен @Invoke
      + methodTwo() — без аннотации
      + methodThree() — помечен @Invoke

   - Каждый метод изменяет соответствующий логический флаг при вызове.

Назначение:
   - Позволяет проверить:
      + Корректность обнаружения аннотированных методов.
      + Корректность их вызова.
      + Отсутствие вызова методов без аннотации.

&nbsp;

> Задание 8 – (Тест @Validate)
...
В ходе выполнения данного задания был разработан класс ValidateProcessorTest.

### ValidateProcessorTest

Класс предназначен для тестирования корректности работы обработчика аннотации @Validate — ValidateProcessor.

Описание:
   - Проверяет корректность извлечения классов из аннотации @Validate.
   - Проверяет выброс исключения IllegalArgumentException при пустом массиве типов.
   - Использует фреймворк JUnit 5.

*Вспомогательные тестовые классы:*

TestClassValid
   - Аннотирован @Validate.
   - Содержит массив типов: String.class и Integer.class.
   - Используется для проверки корректного извлечения классов аннотации.

TestClassEmpty
   - Аннотирован @Validate с пустым массивом.
   - Используется для проверки выброса исключения при отсутствии указанных классов.

*Метод testValidAnnotationExtraction():*

Аннотация: @Test

Описание:
   - Получает аннотацию @Validate через getAnnotation().
   - Проверяет:
      + Аннотация присутствует (assertNotNull).
      + Количество указанных классов равно 2.
      + Классы извлекаются в правильном порядке (String, Integer).

Назначение:
   - Подтверждает корректное извлечение классов из аннотации и правильный порядок элементов.

*Метод testEmptyAnnotationThrowsException():*

Аннотация: @Test

Описание:
   - Проверяет вызов метода ValidateProcessor.process() для класса с пустым массивом типов.
   - Ожидается выброс IllegalArgumentException при отсутствии указанных классов.

Назначение:
   - Подтверждает корректную обработку ошибки пустого массива типов в аннотации.
